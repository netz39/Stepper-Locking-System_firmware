...
protected:
    ...
    inline mcErr err(mcErr code) const
    {
        return (mcErr)error(errModule::MC, (int)code);
    }
...
void MotorControlBase<t>::stepTimerISR()
{
    ...
    while (*(++slave) !=
           nullptr) // move slave motors if required (https://en.wikipedia.org/wiki/Bresenham)
    {
        if ((*slave)->B >= 0)
        {
            (*slave)->doStep();
...
{
    static_assert(sizeof...(steppers) < MaxMotors,
                  "Too many motors used. Please increase MaxMotors in file MotorControlBase.h");

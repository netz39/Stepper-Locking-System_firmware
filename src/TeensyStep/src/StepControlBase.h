...
public:
    ...
    {
        moveAsync(1.0f, steppers...);
    }

    // Move variadic param list of stepper references with speed override e.g.
    // moveAsync(0.2f,m1,m2,m3,m5)
    template <typename... Steppers>
    ...
    template <size_t N>
    void moveAsync(Stepper *(&steppers)[N])
    {
        moveAsync(1.0f, steppers);
    }
    ...
    // Move variadic param list of stepper references with speed override. E.g., move(0.5f,
    // m1,m2,m3,m5)
    template <typename... Steppers>
...
void StepControlBase<a, t>::doMove(int N, float speedOverride)
{
    ...
    std::sort(this->motorList, this->motorList + N,
              Stepper::cmpDelta); // The motor which does most steps leads the movement, move to top
                                  // of list
    ...
    uint32_t targetSpeed =
        std::abs(
            (*std::min_element(this->motorList, this->motorList + N, Stepper::cmpVmin))->vMax) *
        speedOverride; // use the lowest max frequency for the move, scale by relSpeed
    uint32_t pullInSpeed = this->leadMotor->vPullIn;
    uint32_t pullOutSpeed = this->leadMotor->vPullOut;
    uint32_t acceleration =
        (*std::min_element(this->motorList, this->motorList + N, Stepper::cmpAcc))
            ->a; // use the lowest acceleration for the move
    ...
    this->timerField.setStepFrequency(
        accelerator.prepareMovement(this->leadMotor->current, this->leadMotor->target, targetSpeed,
                                    pullInSpeed, pullOutSpeed, acceleration));
...
template <typename a, typename t>
template <size_t N>
void StepControlBase<a, t>::moveAsync(
    float speedOverride, Stepper *(&motors)[N]) // move up to maxMotors motors synchronously
{
